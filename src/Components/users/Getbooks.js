...
function Getbooks() {
  ...
  const handlePrev = () =>{
    ...
    async function reservebook(book){
        const response = await fetch("http://localhost:8080/library_management/reservebook?userid=" + sessionStorage.getItem("uid") +"&bookid=" + book.Id +"&user=Member");
        const data = await response.json();
